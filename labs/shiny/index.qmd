...
Create a simple chatbot with access to OpenAI LLM models.
...
- Accessing API endpoints using httr package
- Using purrr to work with JSON lists

The following R packages are required for this app: `c(shiny, bslib, httr2, purrr, glue)`.

Below is a preview of the complete app.

![](assets/chatbot.webp)

Create a file named **.Renviron** and enter the OpenAI API key in this format 

```
OPENAI_API_KEY="sk-747hSgf64kjsdjaGJ4hD9Ge3lS8Dg"
```

### Custom functions

We need to create a few custom functions to send HTTP requests to the OpenAI server to get a list of models and to send the chat prompts.

```{r}
#| label: chatbot-functions-1
#| eval: false


#' Get a list of available models from the OpenAI API
#' @param api_key OpenAI API key (default is taken from the environment variable
#' @return A character vector containing the names of available models
#' 
list_models <- function(api_key = Sys.getenv("OPENAI_API_KEY")) {
  # Check if API key is available
  if (api_key == "") {
    warning("API key is not set. Please set the OPENAI_API_KEY environment variable.")
    return("gpt-4.1-nano")
  } else {
    ...
    if (resp$status_code >= 400) {
      stop("Request failed with status code: ", resp$status_code)
    }

    # Remove simplifyVector = TRUE to preserve list structure
    models_response <- resp |> resp_body_json()

    # return only models that start with "gpt-" or "o" and are not previews
    models <- models_response$data |>
      purrr::keep(~ grepl("^gpt-[0-9]+|^o[0-9]+", .x$id) && !grepl("preview", .x$id)) |>
      purrr::map_chr("id") |>
      sort()
...
#' @param user_prompt A character string containing the user's prompt
#' @param model The model to use for the chat (default is "gpt-4.1-nano")
#' @param history A list containing the chat history
#' @param system_prompt System prompt to guide the AI's responses
#' @param api_key OpenAI API key
#' @return A character string with OpenAI's response
#'
chat <- function(user_prompt, model = "gpt-4.1-nano", history = NULL, system_prompt = "You are a helpful assitant.", api_key = Sys.getenv("OPENAI_API_KEY")) {
  # Check if API key is available
  if (api_key == "") {
    stop("API key is not set. Please set the OPENAI_API_KEY environment variable.")
  }

  prompt <- prepare_prompt(user_prompt, system_prompt, history)
  base_url <- "https://api.openai.com/v1"
  ...
  # Check for expected response structure before accessing its content
  if (!is.null(openai_chat_response$choices) && length(openai_chat_response$choices) > 0) {
    return(openai_chat_response$choices$message$content)
  } else {
    stop("Unexpected response structure: ", openai_chat_response)
  }
}
```

We have two more functions to format the prompts:

```{r}
#| label: chatbot-functions-2
#| eval: false


#' Prepare the prompt to be sent to the OpenAI API
#' @param user_prompt A character string with the user's prompt
#' @param system_prompt A character string with the system prompt
#' @param history A list containing the chat history
#' @return A list containing the full prompt
#'
prepare_prompt <- function(user_prompt, system_prompt = "You are a helpful assitant.", history = NULL) {
  user_prompt <- list(list(role = "user", content = user_prompt))
  sys_prompt <- list(list(role = "system", content = system_prompt))
  c(sys_prompt, history, user_prompt) |> purrr::compact()
}

#' Update the chat history with the latest user prompt and response
#' @param history A list containing the existing chat history
#' @param user_prompt The latest prompt from the user
#' @param response The response from the OpenAI API
#' @return An updated list containing the new chat history
#'
update_history <- function(history = NULL, user_prompt, response) {
  c(
    history,
    list(
...
Add these functions to a file named **functions.r**.

### App

Then, create a file named **app.r** and add the following:
...
ui <- page_sidebar(
  ...
  sidebar = sidebar(
    open = "closed",
    selectInput("model", "Model", choices = models, selected = "gpt-4.1-nano"),
    textAreaInput("system_prompt", "System prompt", value = "You are a helpful assistant."),
  ),
  layout_columns(
    fill = FALSE,
    col_widths = c(10, 2),
    textAreaInput("prompt", NULL),
    actionButton("chat", NULL, icon = icon("paper-plane"), width = "75px", class = "m-2 btn-secondary")
  ),
...
server <- function(input, output, session) {
  ...
  observe({
    req(input$prompt != "")
    response <- chat(user_prompt = input$prompt, system_prompt = input$system_prompt, history = rv$chat_history)
    rv$chat_history <- update_history(history = rv$chat_history, user_prompt = input$prompt, response = response)
    output$chat_history <- renderUI(
      map(rev(rv$chat_history), \(x) markdown(glue("**{x$role}**: {x$content}")))
    )
...
When a user enters a prompt, the **httr** functions are used to send and receive requests from the OpenAI API endpoint. The **purrr** functions are used to massage the JSON response which is then displayed. Upon entering the next prompt, the new prompt and the old chat history are all together send to the API. Feel free to breakdown the script to understand it better.

The OpenAI API reference documentation can be found [here](https://platform.openai.com/docs/api-reference/introduction).

Do you have any ideas to improve this app? Do you have more features that you would like to add to this app?
